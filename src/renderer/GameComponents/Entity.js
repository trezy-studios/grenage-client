...
class Entity {
  ...
  createBody = () => {
    ...
    Body.setPosition(body, this.options.position)
  ...
  getCurrentFrame = () => {
    ...
    const currentFrame = this.entityData.frames[currentFrameIndex]
    const frameTag = this.entityData.meta.frameTags.find(({ name }) => name === this.getState())
    ...
    return this.entityData.frames[currentFrameIndex].frame
  }

  getCoordinatesToRender = () => {
    const renderBounds = { ...this.getCurrentFrame() }
    // const renderBounds = { ...this.entityData.frames[0].frame }
    const zones = this.zones

    if (zones.length) {
      const zone = zones.find(({ name }) => new RegExp(`::${this.getDirection()}$`, 'gi').test(name))
      // const zone = zones.find(({ name }) => new RegExp(`::south$`, 'gi').test(name))
  ...
  initialize = async () => {
    ...
    const imageLoadPromise = new Promise(resolve => {
      this.imageDownloadStarted = true
    ...
    image.src = this.options.entityData.meta.image.substring(this.options.entityData.meta.image.indexOf(this.baseURL))
  ...
  get environmentalHitboxSlices () {
    return this.options.environmentalHitboxes
  }

  get entityData () {
    return this.options.entityData
  }

  get hitboxSlices () {
    return this.options.hitboxes
  }
  ...
  get size () {
    ...
    return this.entityData.frames[0].sourceSize
  }
